...
class BaseCamera:
    ...
    def extract(self, frame_ids: List[int], 
                mode: str='footage') -> List[np.ndarray]:
        """
        ...
        discarded_ids = [frame_id for frame_id in frame_ids 
                         if frame_id > self.max_frame or frame_id < 0]
        ...
        for video_file in self.video_dict[mode]:
            ...
            while (current_frame_index < len(sorted_frame_ids) and 
                   (sorted_frame_ids[current_frame_index] 
                    < self.video_dict[mode][video_file]["start"])):
            ...
            for idx in range(current_frame_index, len(sorted_frame_ids)):
                ...
                if idx % 100 == 0:
                    log.info(f"Extracting frame {sorted_frame_ids[idx]} "
                             f"from {self.name}...")

                shifted_id = sorted_frame_ids[idx] - start_frame
                if idx % 100 == 0:
                    log.debug(f"Extracting frame {shifted_id} "
                             f"from {video_file}...")
                extracted_frames.append(self._extract_frame(video, shifted_id))
                # if the frame is empty, replace it with a blank frame
                if len(extracted_frames[-1]) == 0:
                    log.warning(f"Frame {shifted_id} is empty, replacing with "
                                "a blank frame...")
                    extracted_frames[-1] = np.zeros((self.calibration.size[0], 
                                                     self.calibration.size[1], 
                                                     3), dtype=np.uint8)

                # Compare the current frame to last frame in extracted_frames
                # If the frames are the same, remove the current frame
                if (len(extracted_frames) > 1 and 
        ...
        extracted_frames = [extracted_frames[index] for index in 
                            unsorted_indices[:len(extracted_frames)]]
...
class Camera(BaseCamera):
    ...
    def get_ground_plane_homography(
            self, 
            input_img_size:Tuple[int, int] = (1920, 1080),
            output_img_size:Tuple[int, int] = (1920, 1080),
        ...
        scale = longest_side / max(bounding_box[1][0], bounding_box[1][1])
        ...
        tx = ((output_img_size[0] - 1)/2) - cx*scale
        ty = ((output_img_size[1] -1)/2) - cy*scale
        ...
        factorx = input_img_size[0] / (K[0,2] * 2)
        factory = input_img_size[1] / (K[1,2] * 2)
...
class MultiviewVids():
    ...
    def get_homographies(self, input_size:tuple = (1920, 1080), 
    ...
    def get_view_ROIs(self) -> List[np.ndarray]:
        ...
        view_ROIs = []
        for cam in self.cams:
            view_ROIs.append(cam.calibration.ROI)
    ...
    def load_gt(self, set_name:str, undisort:bool = True) -> dict:
        ...
        for ann_path in anns_path_list:
            ...
            log.debug(f"Loading annotations for frame {frame_id}...")
                ...
                for ann in ann_data:
                    ...
                    if undisort:
                        feet_reproj = geometry.project_world_to_camera(
                            world_point, calib.K, calib.R, calib.T, calib.dist)
                    else:
                        feet_reproj = geometry.project_world_to_camera(
