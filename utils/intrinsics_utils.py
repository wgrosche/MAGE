...
from support import rgb2gray, rmdir, mkdir, find_images
...
def process_image(filename_image: str, inner_corners_height: int, 
    ...
    ```
    """
    log.info("Processing image {} ...".format(filename_image))
...
def compute_intrinsics(folder_images: str, output_folder:str, args) -> dict:
    ...
    log.info("-" * 50)
    log.info("folder_images:", folder_images)
    log.info("output_folder:", output_folder)
    log.info("description:", description)
    log.info("inner_corners_height:", inner_corners_height)
    log.info("inner_corners_width:", inner_corners_width)
    log.info("square_sizes:", square_sizes)
    log.info("rational_model:", rational_model)
    log.info("alpha:", alpha)
    log.info("force_monotonicity:", force_monotonicity)
    log.info("monotonic_range:", monotonic_range)
    log.info("intrinsic_guess:", intrinsic_guess 
             if len(intrinsic_guess) else False)
    log.info("fix_principal_point:", fix_principal_point)
    log.info("fix_aspect_ratio:", fix_aspect_ratio)
    log.info("zero_tangent_dist:", zero_tangent_dist)
    log.info("criteria_eps:", criteria_eps)
    ...
    if load_keypoints:
        keypoints = read_json(os.path.join(output_folder, "keypoints.json"))
    ...
    log.info(f"Saving to {os.path.join(output_folder, 
                                       'detected_2d_histogram.jpg')}")
    plt.figure()
    ...
    if len(intrinsic_guess):
        intrinsic_guess = read_json(intrinsic_guess)
